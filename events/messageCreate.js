...
async function argumentsHandler(element, args, message) {
    ...
    if (args.length > template.length && template[template.length - 1].type !== "string") {
        message.channel.send(`Un élément innatendu a été rencontré: ${args[args.length - 1]}. Usage: ${usagePrinter(element)}`)
        return false;
    }
    ...
    for (let i = 0; i < template.length; i++) {
        ...
        if (!rawArg) {
            if (required) {
                message.channel.send(`Un argument obligatoire (${name}) est manquant. Usage: ${usagePrinter(element)}`);
                return false;
            }
            else break;
        }
        ...
        switch (type) {
            ...
            case "int":
                ...
                if (!Number.isInteger(intArg)) {
                    message.channel.send(`L'argument ${name} est incorrect. Un int est demandé. Usage: ${usagePrinter(element)}`);
                    return false;
                }
            ...
            case "float":
                ...
                if (Number.isNaN(floatArg)) {
                    message.channel.send(`L'argument ${name} est incorrect. Un float est demandé. Usage: ${usagePrinter(element)}`);
                    return false;
                }
            ...
            case "user":
                ...
                if (!member) {
                    message.channel.send(`L'argument ${name} est incorrect. Un utilisateur (mention ou id.) est demandé. Usage: ${usagePrinter(element)}`);
                    return false;
                }
                betterArgs.push(member);
                break;
            default:
                message.channel.send(`Une erreur lors de la gestion des arguments de la commande est survenue. Type demandé: ${type} pour l'argument ${name} de la commande ${element.name}. Usage: ${usagePrinter(element)}`);
        }
    }
